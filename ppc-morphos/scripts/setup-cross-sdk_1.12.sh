#!/bin/sh
#
# setup-cross-sdk.sh version 1.12-beta Copyright 2013-2026 Mark Olsen 
#
...
#
# History:
# 1.12-beta - 20260531 - BeWorld
#  - Added local support for GCC 15 from sdk-source-20260529.tar.xz.
#  - Added the --gcc15 option and automatic GCC 15 detection when no GCC version is selected.
#  - Extended source extraction and final compiler symlink setup to handle GCC 15.
#  - Reworked the GCC 11 build block into a shared GCC 11/GCC 15 build loop.
#  - Added a Makefile fixup for fix-saveds.diff and objc-saveds.diff so they are applied with patch -p0 instead of patch -p1.
#  - Builds GCC 11/GCC 15 with SDK_RELEASE_ID=1 and MAKEINFO=true to avoid release-id and documentation-generation issues.
#  - Kept the original SDK .lha plus sdk-source-YYYYMMDD.tar.xz workflow.
#
# Tested only with Debian 13
#
# 1.11 - 20220406
...
patch_makefile_saveds_strip_level()
{
	local GCC_VERSION="$1"

	if test -f "${TMPDIR}/gcc${GCC_VERSION}/Makefile"
	then
		echo "Fixing saveds patch strip level for GCC ${GCC_VERSION}." >>"$CURDIR/setup-cross-sdk.log"

		sed -i \
			-e 's,(cd \([^)]*\) && patch -p1) <fix-saveds.diff,(cd \1 \&\& patch -p0) <fix-saveds.diff,g' \
			-e 's,(cd \([^)]*\) && patch -p1) <objc-saveds.diff,(cd \1 \&\& patch -p0) <objc-saveds.diff,g' \
			"${TMPDIR}/gcc${GCC_VERSION}/Makefile" >>"$CURDIR/setup-cross-sdk.log" 2>&1 || ERROR=1
	fi
}
...
echo "Using SDK: $SDK"
echo "Using SDK source: $SDKSOURCE"
...
if [ "$GCC4" != "y" ] && [ "$GCC5" != "y" ] && [ "$GCC6" != "y" ] && [ "$GCC7" != "y" ] && [ "$GCC8" != "y" ] && [ "$GCC9" != "y" ] && [ "${GCC10}" != "y" ] && [ "${GCC11}" != "y" ] && [ "${GCC15}" != "y" ]
then
	SDKGCCVERSION=gcc`tar -tf "$CURDIR/$SDKSOURCE" | sed "s,^sdk-source/,," | grep "^gcc\(4\|5\|6\|7\|8\|9\|10\|11\|15\)/" | sed "s,/.*,," | sed "s,^gcc,," | sort -n | uniq | tail -n 1`
...
if [ "$GCC15" = "y" ]
then
	GCCVERSIONS="$GCCVERSIONS ${SOURCETARBALLPREFIX}gcc15"
fi
...
for GCCN in 11 15
do
	eval GCCSEL=\$GCC${GCCN}
	if [ "$GCCSEL" = "y" ]
	then
		# Build GCC ${GCCN}
		echo "Building GCC ${GCCN}."

		sed -i -e 's/--with-\(mpfr\|gmp\|mpc\)="[^"]*"//g' -e "s/--program-suffix=-8/--program-suffix=-${GCCN}/g" -e 's/--enable-threads=posix/--enable-threads=morphos/' "$TMPDIR/gcc${GCCN}/Makefile"
		patch_atomic ${GCCN}
		patch_makefile_fix_fenv_for_crossbuild ${GCCN}
		patch_makefile_saveds_strip_level ${GCCN}
		patch_gcc_lib_path_fixup ${GCCN}

		test -z "$ERROR" && (cd "$TMPDIR/gcc${GCCN}" && make $MAKEPARALLEL >>"$CURDIR/setup-cross-sdk.log" 2>&1) || ERROR=1
		test -z "$ERROR" && (cd "$TMPDIR/gcc${GCCN}" && make install >>"$CURDIR/setup-cross-sdk.log" 2>&1) || ERROR=1

		if [ ! -z "$ERROR" ]
		then
			echo "Failed to build GCC ${GCCN} for whatever reason. You're unfortunately on"
			echo "your own now. Check \"$CURDIR/setup-cross-sdk.log\" for details."
			exit 1
		fi
	fi
...
if [ "$GCC15" = "y" ]
then
	ln -sf /gg/bin/ppc-morphos-gcc-15 /gg/bin/ppc-morphos-gcc
	ln -sf /gg/bin/ppc-morphos-g++-15 /gg/bin/ppc-morphos-g++
elif [ "$GCC11" = "y" ]
then
